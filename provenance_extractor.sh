...
export IMAGE_URL=$(tkn pr describe --last -o jsonpath="{.status.results[2].value}")
gcloud artifacts docker images describe $IMAGE_URL --show-all-metadata --format json | jq -r '.provenance_summary.provenance[0].envelope.payload' | tr '\-_' '+/' | base64 -d | jq > provenance
gcloud artifacts docker images describe $IMAGE_URL --show-all-metadata --format json | jq -r '.provenance_summary.provenance[0].envelope.signatures[0].sig' | tr '\-_' '+/' | base64 -d > signature